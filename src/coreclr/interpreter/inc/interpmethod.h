...
//
// The includer must define INTERPRETER_COMPILER_INTERNAL when it builds the
// representation as a compiler (so method handles are CORINFO_METHOD_HANDLE);
// otherwise the VM (DPTR(MethodDesc)) form is used.
