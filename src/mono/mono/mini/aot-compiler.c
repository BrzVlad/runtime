...
emit_llvm_file (MonoAotCompile *acfg)
{
	...
	if (acfg->aot_opts.llvm_only) {
		...
		// FIXME: arch
		command = g_strdup_printf ("%s -fexceptions -fpic -O0 -fno-optimize-sibling-calls -Wno-override-module -c -o \"%s\" \"%s.opt.bc\"", acfg->aot_opts.clangxx, acfg->llvm_ofile, acfg->tmpbasename);
